"""
GitHub Agent using LangGraph's create_react_agent.
This agent handles GitHub operations via MCP servers.

Debug Configuration:
- Set environment variable GITHUB_AGENT_DEBUG=false to disable debug output
...
class GitHubAgentState(MessagesState):
...
async def github_node(state: GitHubAgentState, config: RunnableConfig):
    """
    ...
    try:
        ...
        api_key = await get_user_api_key(int(user_id), "github", AGENT_NAME)
        if not api_key:
...
async def create_graph():
    ...
    try:
        ...
        workflow.set_entry_point("github_node")
