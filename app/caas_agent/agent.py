"""
CaaS Agent using LangGraph's create_react_agent.
This agent manages Compute as a Service operations.

Debug Configuration:
- Set environment variable CAAS_AGENT_DEBUG=false to disable debug output
...
AGENT_NAME = "CAAS_AGENT"

class CaaSAgentState(MessagesState):
    """State for the CaaS Agent"""
    pass
...
async def caas_node(state: CaaSAgentState, config: RunnableConfig):
    """
...
async def create_graph():
    ...
    try:
        ...
        workflow = StateGraph(CaaSAgentState)
        workflow.add_node("caas_node", caas_node)
        workflow.set_entry_point("caas_node")
        workflow.add_edge("caas_node", END)
